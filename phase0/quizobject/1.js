...
function ganjilGenapAsianGames(date, data) {
    //code here
        ...
        for (let i = 0; i < data.length; i++) {
            let vehicleType = data[i].type
            let vehicleNumber =  data[i].plat
            
            if(vehicleType === "Mobil"){
                let vehicleSerial = vehicleNumber.match(regex)
                carSerial.push(vehicleSerial);
        ...
        for (let j = 0; j < carSerial.length; j++) {
            let len = carSerial[j][0].length
            let lastDigitNumber = carSerial[j][0][len-1]
            
            if(lastDigitNumber%2==0){
                numEven.push(lastDigitNumber)

            }else{
                numOdd.push(lastDigitNumber)
            }
            
        }
      
        if(date%2==0){
            return numOdd.length
        }else{
