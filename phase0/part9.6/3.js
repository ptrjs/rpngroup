...
dari toko X tersebut yang berisikan info nama barang, siapa saja yang membeli, 
...
function countProfit(shoppers) {
    ...
    for(let i = 0 ; i < listBarang.length; i++){
        let nameBarang = listBarang[i][0]
        let priceBarang = listBarang[i][1]
        let stockBarang = listBarang[i][2]

        let shoppersList = []
        let leftOver = stockBarang
        let totalProfit = 0
        for(let j = 0 ; j < shoppers.length; j++){
            if(shoppers[j].product===nameBarang && shoppers[j].amount < stockBarang){
                shoppersList.push(shoppers[j].name)
                totalProfit += shoppers[j].amount*priceBarang
